...
public static class MapTextureGenerator
{
    ...
    public static Texture2D GenerateTexture(MapGraph map, int meshSize, int textureSize, List<MapNodeTypeColor> colours, bool drawBoundries, bool drawTriangles, bool drawCenters, Vector2 centerPoint, MapGraph.MapNodeType wonPlayer)
    {
        CreateDrawingMaterial();
        var texture = RenderGLToTexture(map, textureSize, meshSize, drawingMaterial, colours, drawBoundries, drawTriangles, drawCenters, centerPoint, wonPlayer);

        return texture;
    }

    // Modified by Kyuji
    public static Texture2D GenerateTexture(MapGraph map, int meshSize, int textureSize, List<MapNodeTypeColor> colours, bool drawBoundries, bool drawTriangles, bool drawCenters, List<Vector2> centerPoints)
    {
        CreateDrawingMaterial();
        var texture = RenderGLToTexture(map, textureSize, meshSize, drawingMaterial, colours, drawBoundries, drawTriangles, drawCenters, centerPoints);

        return texture;
    }

    // Modified by Kyuji
    private static Texture2D RenderGLToTexture(MapGraph map, int textureSize, int meshSize, Material material, List<MapNodeTypeColor> colours, bool drawBoundries, bool drawTriangles, bool drawCenters, List<Vector2> centerPoints)
    {
        var renderTexture = CreateRenderTexture(textureSize, Color.white);

        // render GL immediately to the active render texture //
        DrawToRenderTexture(map, material, textureSize, meshSize, colours, drawBoundries, drawTriangles, drawCenters, centerPoints);

        return CreateTextureFromRenderTexture(textureSize, renderTexture);
    }

    private static Texture2D RenderGLToTexture(MapGraph map, int textureSize, int meshSize, Material material, List<MapNodeTypeColor> colours, bool drawBoundries, bool drawTriangles, bool drawCenters, Vector2 centerPoint, MapGraph.MapNodeType wonPlayer)
    {
        var renderTexture = CreateRenderTexture(textureSize, Color.white);

        // render GL immediately to the active render texture //
        DrawToRenderTexture(map, material, textureSize, meshSize, colours, drawBoundries, drawTriangles, drawCenters, centerPoint, wonPlayer);
    ...
    private static void DrawToRenderTexture(MapGraph map, Material material, int textureSize, int meshSize, List<MapNodeTypeColor> colours, bool drawBoundries, bool drawTriangles, bool drawCenters, Vector2 centerPoint, MapGraph.MapNodeType wonPlayer)
    {
        material.SetPass(0);
        GL.PushMatrix();
        GL.LoadPixelMatrix(0, meshSize, 0, meshSize);
        ...
        */
        ...
        DrawNodeTypes(map, coloursDictionary, centerPoint, wonPlayer);
    ...
    // Modified by Kyuji
    private static void DrawToRenderTexture(MapGraph map, Material material, int textureSize, int meshSize, List<MapNodeTypeColor> colours, bool drawBoundries, bool drawTriangles, bool drawCenters, List<Vector2> centerPoints)
    {
        material.SetPass(0);
        GL.PushMatrix();
        GL.LoadPixelMatrix(0, meshSize, 0, meshSize);
        ...
        */
        ...
        DrawNodeTypes(map, coloursDictionary, centerPoints);
    ...
    private static void DrawNodeTypes(MapGraph map, Dictionary<MapGraph.MapNodeType, Color> colours, Vector2 centerPoint, MapGraph.MapNodeType wonPlayer)
    {
        GL.Begin(GL.TRIANGLES);
        foreach (var node in map.nodesByCenterPosition.Values)
        {
            if (Vector3.Distance(node.centerPoint, new Vector3(centerPoint.x, 0f, centerPoint.y)) < 0.1f)
            {
                node.nodeType = wonPlayer;
                var color = colours.ContainsKey(node.nodeType) ? colours[node.nodeType] : Color.red;
                GL.Color(color);
            }
            else
    ...
    // Modified by Kyuji
    private static void DrawNodeTypes(MapGraph map, Dictionary<MapGraph.MapNodeType, Color> colours, List<Vector2> centerPoints)
    {
        GL.Begin(GL.TRIANGLES);
        foreach (var node in map.nodesByCenterPosition.Values)
        {
            foreach (Vector2 centerPoint in centerPoints)
            {
                if (Vector3.Distance(node.centerPoint, new Vector3(centerPoint.x, 0f, centerPoint.y)) < 0.1f)
                {
